...
# )
@pytest.fixture
def minigame_reward_chest_emu(tmp_path: Path, emulator: AbstractEmulatorWrapper, request):
    rom_path = str(tmp_path / f'{tmp_path.name}.nds')

    emulator.open(rom_path)

    return emulator


def test_minigame_reward_chests(minigame_reward_chest_emu: AbstractEmulatorWrapper):
    ...
    # Talk to goron
    minigame_reward_chest_emu.touch_set_and_release((165, 65), 20)  # touch goron
